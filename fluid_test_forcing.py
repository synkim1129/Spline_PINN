...
logger = Logger(get_hyperparam_fluid(params) + ' with_forcing',use_csv=False,use_tensorboard=False)
...
v_obs = torch.zeros(1,2,params.width,params.height)

cv2.namedWindow('obs', cv2.WINDOW_NORMAL)
text = np.zeros((params.height*2, params.width*2, 3), np.uint8) + 255

# v_obs 위치와 값 랜덤하게 설정 (필요시 수정하여 사용)
x_indices = torch.randperm(params.width-2)[0:params.n_forcing]+1
y_indices = torch.randperm(params.height-2)[0:params.n_forcing]+1
for i in range(params.n_forcing):
	x_obs = x_indices[i]
	y_obs = y_indices[i]
	v_obs[0,0,y_obs,x_obs] = 2 * torch.rand(1) - 1
	v_obs[0,1,y_obs,x_obs] = 2 * torch.rand(1) - 1
	cv2.putText(text, f'v_obs at {(x_obs.item(), y_obs.item())}: {v_obs[0, 0, y_obs, x_obs].item():.2f}, {v_obs[0, 1, y_obs, x_obs].item():.2f}', (10, 30 + 30 * i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0))
cv2.imshow('obs', text)
v_obs = toCuda(v_obs)
...
while not exit_loop:
	...
	for i in range(params.average_sequence_length):
		...
		if i%1==0:
			...
			for i in range(params.n_forcing):
				x, y = x_indices[i].item(), y_indices[i].item()
				cv2.circle(image, (int((y-0.5) * resolution_factor), int((x-0.5) * resolution_factor)), 20, (0, 0, 255), 4)
			if save_movie:
